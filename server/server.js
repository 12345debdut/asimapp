...
const payment = require("./payments/router/paymentRouter.ts");
...
app.use(
  "/exam",
  express.static(path.join(__dirname, "../client", "examclient", "build"))
);
app.use(
  "/practice",
  express.static(path.join(__dirname, "../client", "practiceclient", "build"))
);
...
app.get("/practice/*", (req, res) => {
  res.sendFile(
    path.join(__dirname, "../client", "practiceclient", "build", "index.html")
  );
});

app.get("/exam/*", (req, res) => {
  res.sendFile(
    path.join(__dirname, "../client", "examclient", "build", "index.html")
  );
});
app.get("*", (req, res) => {
  res.sendFile(
    path.join(__dirname, "../client", "mainclient", "build", "index.html")
  );
});
app.listen(process.env.PORT || 5000, () => {
  console.log(
    `Server is listening ${process.env.PORT ? process.env.PORT : 5000}`
  );
});
